...
<div class="jumbotron jumbotron-fluid">
  <div class="container">
    <h1>Your wishlist</h1> 
    <p>Keeping all your favorites...</p> 
  </div>
</div>
</section>
...
function updateWishlist(userId) {
	...
	requestGet(data, function(obj) {
		...
		$.each(obj.wishlist, function(key, value) {
...
function deleteWishlist(userId, wishlistId) {
	...
	requestPost(data, function(obj) {
		...
		$(".sidebar-nav span.wishlist").html(counter);
		$(".card-columns").find("[data-itemId='" + wishlistId + "']").remove();
	});	
}
...
$('div.card-columns').on('click', 'button.remove', function() {
	...
	if (confirm('Are you sure deleting this item?')) {
		deleteWishlist(<?php echo $userId; ?>, itemId);
	}
});
</script>
